    ...
    <View style={styles.card}>
      ...
      <View style={styles.footer}>
        <Text style={styles.metrics}>
          {discovery.stopCount} stops |{" "}
          {durationLabel(discovery.durationMinutes)}
        </Text>
        <Pressable style={styles.action} disabled={planning} onPress={onPlan}>
          {planning ? (
            ...
            <Text style={styles.actionText}>
              {savedTripId ? "Open my trip" : "Plan this trip"}
            </Text>
          )}
          {!planning ? (
            <Ionicons name="arrow-forward" size={16} color={colors.kopi} />
          ) : null}
        </Pressable>
      </View>
    </View>
