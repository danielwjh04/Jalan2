...
export function voiceRouter(config: Config, tts: TextToSpeech): Router {
  ...
  router.get('/voice/brief/:itineraryId', (req, res) => {
    ...
    if (!itinerary?.booking) {
      res.status(404).json({ error: `No briefable itinerary ${req.params.itineraryId}` });
      return;
    }
    const lang: BriefLang = req.query.lang === 'ms' ? 'ms' : 'en';
  ...
  router.get('/voice/phrases', (_req, res) => {
    void Promise.all(
      PHRASE_CLIPS.map(
        async (phrase): Promise<PhraseClipResponse> => ({
          ...phrase,
