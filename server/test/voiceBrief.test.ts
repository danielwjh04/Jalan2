...
describe('composeBrief', () => {
  ...
  it('produces different text per language', () => {
    expect(composeBrief(booking(), 'en')).not.toBe(composeBrief(booking(), 'ms'));
  ...
  it('mentions the evidenced price and skips it when null', () => {
    ...
    expect(composeBrief(booking({ price_myr: null }), 'en')).not.toContain('ringgit per person');
  });

  it('picks water guidance for boat and dive activities', () => {
    expect(composeBrief(booking(), 'en')).toContain('life jacket');
    expect(composeBrief(booking({ activity: 'Street food walk' }), 'en')).not.toContain(
      'life jacket',
    );
  });

  it('always closes with the advisory line', () => {
    expect(composeBrief(booking(), 'en')).toContain('general advisory guidance');
